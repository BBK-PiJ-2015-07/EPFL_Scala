...
object Huffman {
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case x :: xs =>
        val fork = List(makeCodeTree(trees(0), trees(1)))
        msort((fork ::: (trees drop 2)))( (y, z) => weight(y) < weight(z))
      case _ => trees
    }
   ...
   */
    ...
    case Nil => throw new Exception("Nil or empty list passed to createCodeTree")
    case x :: xs =>
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      def lookup(remaining: CodeTree)(letter: Char): List[Bit] = remaining match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) => if(chars(left) contains letter) 0 :: lookup(left)(letter) else 1 :: lookup(right)(letter)
      }
  ...
  //Solution for below from here: https://github.com/tonyskn/coursera-scala/blob/master/w4-patmat/src/main/scala/patmat/Huffman.scala
